...
func (s *BotServiceImpl) CreateBot(ctx context.Context, req *bot.CreateBotReq) (resp *bot.CreateBotRes, err error) {
	botId, err := s.botService.CreateBot(req.CreatorId, req.Name, req.AvatarUrl, req.SystemPrompt, req.ApiKey, req.Model)
	if err != nil {
...
func (s *BotServiceImpl) GetSystemBot(ctx context.Context, req *bot.GetSystemBotReq) (resp *bot.GetSystemBotRes, err error) {
	b, err := s.botService.GetSystemBot()
	if err != nil {
		klog.CtxErrorf(ctx, "查询系统Bot时发生系统错误: %v", err)
		return &bot.GetSystemBotRes{Success: false}, err
	}
	return &bot.GetSystemBotRes{Success: true, BotId: b.ID}, nil
}
