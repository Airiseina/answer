...
func main() {
	...
	// Docker
	case "infra-up":
		dockerCompose("up", "-d", "mysql", "postgres", "qdrant", "garnet", "kafka-1", "kafka-2", "etcd", "seaweedfs-master", "seaweedfs-volume", "seaweedfs-filer", "traefik", "jaeger", "otel-collector", "prometheus", "grafana", "searxng")
	case "infra-down":
		dockerCompose("down")
	case "docker-up":
		dockerCompose("up", "-d")
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("Docker:")
	fmt.Println("  infra-up              仅启动基础设施 (DB/Kafka/Qdrant/etcd/…)")
